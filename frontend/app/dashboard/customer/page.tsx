...
export default function CustomerDashboard() {
  ...
  const activeLoans = loans?.loans?.filter((loan: any) => 
  ...
  return (
    <div className="p-6 space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-[#141414]">Customer Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <GlassCard className="p-6">
          <h3 className="text-lg font-semibold text-[#141414] mb-4">Recent Loans</h3>
          <div className="space-y-3">
            {loans?.loans?.slice(0, 3).map((loan: any) => (
              <div key={loan.loan_id} className="flex items-center justify-between p-3 bg-[#EFF0F2] rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-[#141414]/70 capitalize">{loan.status}</p>
                </div>
                <div className={`px-2 py-1 rounded-full text-xs ${
                  loan.status === 'approved' ? 'bg-[#EEC643] text-[#141414]' :
                  loan.status === 'pending' ? 'bg-[#0D21A1] text-[#EFF0F2]' :
